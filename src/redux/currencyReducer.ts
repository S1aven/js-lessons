...
// Пример с дополнительным действием
// export const selectItemById = (state: IGlobalState) => (id: string) => state.currency.currencies.find();
// export const selectItemById = (state: IGlobalState) => state.currency.currencies.filter(el => el.status === state.currency.currencies.status)
